Subroutine loop(l_t,l_k,t,k,stt,stk, omega,PEg,exce,ne,nh, &
     mu,damp,h,V,   pulse_d,pulse_w)
  ...
  Real(DP), Intent(in) :: pulse_d, pulse_w
  ...
  Do j2 = 2,l_t
     Do j1 =1,l_k
        A(j1) = Sum(V(j1, :) * pp(j2 - 1, :) * k) * stk

        Ef1 = Exp( -( (t(j2-1) - pulse_d) ** 2) / (2 * pulse_w ** 2))
        Ef2 = Exp( -( (t(j2-1)+stt/2.0_DP - pulse_d) ** 2) / (2 * pulse_w ** 2))
        Ef3 = Ef2
        Ef4 = Exp( -( (t(j2-1)+stt - pulse_d) ** 2) / (2 * pulse_w ** 2))
        ...
        P(j2) = P(j2) +  2.0_DP * 3.1416 / 1.0 * mu(j1) * k(j1) * pp(j2,j1) * stk
