...
def polarization(fff, params, bs, Ef_h, Ef_e, Tempr, conc, V, E_field):
    ...
    for j2 in range(1, l_t):
        for j1 in range(l_k):
            ...
            RS = -1j * (omega[j1] - Eg / const.h - exce[j1] / const.h) * pp[j2 - 1, j1] - \
                 1j * (ne[j1] - nh[j1]) * (mu[j1] * E_field(t[j2 - 1]) + A[j1]) / const.h - \
                 damp * pp[j2 - 1][j1] / const.h

            kk1 = RS

            kk2 = -1j * (omega[j1] - Eg / const.h - exce[j1] / const.h) * (pp[j2 - 1][j1] + stt * kk1 / 2.0) - \
                  1j * (ne[j1] - nh[j1]) * (mu[j1] * E_field(t[j2 - 1] + stt / 2) + A[j1]) / const.h - \
                  damp * (pp[j2 - 1][j1] + stt * kk1 / 2.0) / const.h

            kk3 = -1j * (omega[j1] - Eg / const.h - exce[j1] / const.h) * (pp[j2 - 1][j1] + stt * kk2 / 2.0) - \
                  1j * (ne[j1] - nh[j1]) * (mu[j1] * E_field(t[j2 - 1] + stt / 2) + A[j1]) / const.h - \
                  damp * (pp[j2 - 1][j1] + stt * kk2 / 2.0) /const. h

            kk4 = -1j * (omega[j1] - Eg / const.h - exce[j1] / const.h) * (pp[j2 - 1][j1] + stt * kk3) - \
                  1j * (ne[j1] - nh[j1]) * (mu[j1] * E_field(t[j2 - 1] + stt) + A[j1]) / const.h - \
                  damp * (pp[j2 - 1][j1] + stt * kk3) / const.h
